...
import { IssueRepository } from '../src/db/issueRepository';
...
describe('IssueService', () => {
  ...
  beforeEach(() => {
    ...
    (issueService as any).issueRepository = issueRepository;
  });
  ...
  it('should get an issue by ID', async () => {
    const mockIssue: Issue = { issue_id: 1, user_id: '1', department_id: 1, location_id: null, category_id: 1, content: 'Test Issue', resolved_at: null, is_anonymous: false, created_at: '2022-01-01', sentiment: 'neutral' };
  ...
  describe('createIssue', () => {
    it('should create a new issue when all required fields are provided', async () => {
      const newIssue: Partial<Issue> = { user_id: '1', department_id: 1, location_id: null, category_id: 1, content: 'New Issue', resolved_at: null, is_anonymous: false, sentiment: 'neutral' };
      const createdIssue: Issue = { ...newIssue, issue_id: 1, created_at: '2022-01-01' } as Issue;
    ...
    it('should throw an error when required fields are missing', async () => {
      ...
      await expect(issueService.createIssue(newIssue)).rejects.toThrowError('Missing required fields for creating an issue');
    ...
    it('should throw an error when content exceeds the maximum length', async () => {
      const newIssue: Partial<Issue> = { user_id: '1', department_id: 1, location_id: null, category_id: 1, content: 'A'.repeat(501), resolved_at: null, is_anonymous: false, sentiment: 'neutral' };

      await expect(issueService.createIssue(newIssue)).rejects.toThrowError('Issue content exceeds the maximum length of 500 characters');
      expect(issueRepository.createIssue).not.toHaveBeenCalled();
    });
  });
  ...
  it('should update an existing issue', async () => {
    ...
    const updatedIssue: Issue = { issue_id: 1, user_id: '1', department_id: 1, location_id: null, category_id: 1, content: 'Updated Issue', resolved_at: null, is_anonymous: false, created_at: '2022-01-01', sentiment: 'neutral' };
