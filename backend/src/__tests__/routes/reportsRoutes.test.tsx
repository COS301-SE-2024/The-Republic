...
describe("Reports Routes", () => {
  ...
  it("should call getAllIssuesGroupedByResolutionStatus controller", async () => {
    (
      reportsController.getAllIssuesGroupedByResolutionStatus[0] as jest.Mock
    ).mockImplementation((req, res) => res.status(200).json({}));

    const response = await request(app).post(
      "/reports/groupedResolutionStatus",
    );

    expect(response.status).toBe(200);
    expect(
      reportsController.getAllIssuesGroupedByResolutionStatus[0],
    ).toHaveBeenCalled();
  });
});
