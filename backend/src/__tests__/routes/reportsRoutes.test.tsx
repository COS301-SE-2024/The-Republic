...
describe('Reports Routes', () => {
  ...
  describe('POST /reports/groupedResolutionStatus', () => {
    it('should call getAllIssuesGroupedByResolutionStatus controller', async () => {
      ...
      (reportsController.getAllIssuesGroupedByResolutionStatus as jest.Mock).mockImplementation((req, res) => res.status(200).json({}));

      const response = await request(app).post('/reports/groupedResolutionStatus').send();

      expect(response.status).toBe(200);
      expect(reportsController.getAllIssuesGroupedByResolutionStatus).toHaveBeenCalled();
    });
  });
});
