...
describe('Visualization Controller', () => {
  ...
  beforeEach(() => {
    ...
    (VisualizationService as jest.MockedClass<typeof VisualizationService>).mockImplementation(() => mockVisualizationService);
    
    // Mock the VisualizationService constructor to return our mock instance
    jest.spyOn(VisualizationService.prototype, 'getVizData').mockImplementation(mockVisualizationService.getVizData);
  });

  it('should call sendResponse for getVizData', async () => {
    await visualizationController.getVizData(mockRequest as Request, mockResponse as Response);
