...
export const getOrganizationResolutions = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const response =
        await issueService.getOrganizationResolutions(organizationId);
      sendResponse(res, response);
    } catch (err) {
      handleError(res, err);
    }
...
export const respondToResolution = async (req: Request, res: Response) => {
  try {
    const { resolutionId, issueId, userId, accept, satisfactionRating } =
      req.body;
    const response = await issueService.respondToResolution(
      resolutionId,
      issueId,
      userId,
      accept,
...
export const getResolutionsForIssue = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const response = await issueService.getResolutionsForIssue(
        parseInt(issueId),
      );
      sendResponse(res, response);
    } catch (err) {
...
export const hasUserIssuesInCluster = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const response = await issueService.hasUserIssuesInCluster(
        userId,
        clusterId,
      );
      sendResponse(res, response);
...
export const getUserIssueInCluster = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const issue = await issueService.getUserIssueInCluster(
        user_id,
        clusterId,
      );
      ...
      if (issue) {
        ...
        return res
          .status(404)
          .json({ error: "User issue not found in the cluster" });
      }
    } catch (error) {
...
export const getRelatedIssues = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const response = await issueService.getRelatedIssues(
        parseInt(issueId),
        userId,
      );
      sendResponse(res, response);
