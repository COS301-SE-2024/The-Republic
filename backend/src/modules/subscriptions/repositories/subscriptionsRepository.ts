...
export default class SubscriptionsRepository {
  async issueSubscriptions({ user_id, issue_id }: Partial<SubsParams>) {
    if (!user_id || !issue_id) {
      throw APIError({
        code: 400,
        success: false,
    ...
    if (selectData && !selectError) {
      if (selectData.issues.includes(issue_id?.toString())) {
        const updatedIssues = selectData.issues.filter(
          (issue: string) => issue !== issue_id,
        );
        ...
        if (updateError || !updateData) {
          ...
          throw APIError({
            ...
            success: false,
            error:
              "An unexpected error occurred while updating the subscription. Please try again later.",
          });
        }
      ...
      if (insertError || !insertData) {
        ...
        throw APIError({
          ...
          success: false,
          error:
            "An unexpected error occurred while creating the subscription. Please try again later.",
        });
      }
  ...
  async categorySubscriptions({ user_id, category_id }: Partial<SubsParams>) {
    ...
    if (selectData && !selectError) {
      if (selectData.categories.includes(category_id?.toString())) {
        const updatedCategories = selectData.categories.filter(
  ...
  async locationSubscriptions({ user_id, location_id }: Partial<SubsParams>) {
    const { data: selectData, error: selectError } = await supabase
      .from("subscriptions")
      .select("locations")
      .eq("user_id", user_id)
    ...
    if (selectData && !selectError) {
      if (selectData.locations.includes(location_id?.toString())) {
        const updatedlocations = selectData.locations.filter(
  ...
  async getSubscriptions({ user_id }: Partial<SubsParams>) {
  ...
  async getNotifications({ user_id }: Partial<SubsParams>) {
    ...
    const { data: issueData, error: issueError } = await supabase.from("issue")
      .select(`
        *,
        issue_id,
        user_id,
    ...
    const { data: pointsData, error: pointsError } = await supabase.from(
      "points_history",
    ).select(`
        user_id,
        action,
    ...
    issueData.forEach((issue) => {
      ...
      if (
        subIssues.includes(issueIdStr) ||
        subCategories.includes(categoryIdStr) ||
        subLocations.includes(locationIdStr) ||
      ...
      if (resolution) {
        if (
          subIssues.includes(issueIdStr) ||
          resolution.resolver_id === user_id
        ) {
          filteredNotifications.push({
            type: "resolution",
