    ...
    cy.get('body').then($body => {
      if ($body.find('[data-testid="comment"]').length > 0) {
        cy.get('[data-testid="comment"]').first().within(() => {
          cy.get('button').contains(/Reply/i).click();
        });
        cy.get('textarea[data-testid="comment-input"]').should('be.visible').type("This is a test reply");
        cy.get('button[type="submit"]').click();
        cy.wait('@getIssues', { timeout: 200000 });
        cy.contains("This is a test reply").should("be.visible");
      } else {
        cy.log('No comments found to reply to');
      }
    });
  });
