...
jest.mock('@/components/Issue/Issue', () => jest.fn(() => <div>Mocked Issue</div>));
jest.mock('@/components/Comment/CommentList', () => jest.fn(() => <div>Mocked CommentList</div>));
jest.mock('@/components/Comment/AddCommentForm', () => jest.fn(() => <div>Mocked AddCommentForm</div>));
...
describe('Issue Page', () => {
  ...
  it('renders issue details and components when data is loaded', async () => {
    ...
    global.fetch = jest.fn(() =>
      Promise.resolve({
        ok: true,
        json: jest.fn().mockResolvedValue({ success: true, data: { id: '1', title: 'Test Issue' } } as unknown) as unknown,
        headers: new Headers(),
        redirected: false,
        status: 200,
        statusText: 'OK',
